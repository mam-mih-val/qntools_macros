...
void run8_pi_neg_correct_clean( std::string list, 
  ...
  auto dd=d
          ...
          .Define( "trPiNegY", rapidity_generator(PI_POS_M, Y_CM), {"pz", "pq"} )
  ...
  correction_task.SetTrackVariables({
                                            std::regex("tr(Pt|Px|Py|Eta|Phi|Charge|PiNegY|DcaR|Chi2Ndf|Nhits|FhcalX|FhcalY|StsNhits|StsChi2)"),
                                    });
  ...
  pi_neg.AddCut( "trStsNhits", [](double nhits){
    return nhits > 5.5;
  }, "cut on fake tracks" );
  pi_neg.AddCut( "trDcaR", [](double dca){
    return dca < 5.0;
  }, "DCA cut" );
  pi_neg.AddCut( "trStsChi2", [](double chi2){
    return chi2 < 5.0;
  }, "cut on chi2 in sts" );
