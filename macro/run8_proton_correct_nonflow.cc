...
void run8_proton_correct_nonflow( std::string list, 
  ...
  auto dd=d
          ...
          .Define("trShuffledPhi", function_random_shuffle_phi, {"trMom"})
  ...
  correction_task.SetTrackVariables({
                                            std::regex("tr(Pt|Px|Py|Eta|Phi|ShuffledPhi|NsigmaProton|NsigmaProton400|NsigmaProton700|Charge|ProtonY|DcaR|Chi2Ndf|Nhits|Weight|WeightTof400|WeightTof700|FhcalX|FhcalY|StsNhits|StsChi2)"),
                                    });
  ...
  nonflow_proton.AddCut( "trNsigmaProton", [](double n_sigma){
    return n_sigma < 3;
  }, "nonflow_proton cut" );
  nonflow_proton.AddCut( "trFhcalX", [](double pos){
    return pos < -30.0 || pos > 160;
  }, "cut on x-pos in fhcal plane" );
  nonflow_proton.AddCut( "trFhcalY", [](double pos){
    return pos < -60.0 || pos > 60;
  }, "cut on y-pos in fhcal plane" );
  nonflow_proton.AddCut( "trStsNhits", [](double nhits){
    return nhits > 5.5;
  }, "cut on fake tracks" );
  nonflow_proton.AddCut( "trDcaR", [](double dca){
    return dca < 5.0;
  }, "DCA cut" );
  nonflow_proton.AddCut( "trStsChi2", [](double chi2){
    return chi2 < 5.0;
  }, "cut on chi2 in sts" );
  nonflow_proton.AddHisto2D({{"trProtonY", 100, -0.5, 1.5}, {"trPt", 100, 0.0, 2.0}});
  correction_task.AddVector(nonflow_proton);
