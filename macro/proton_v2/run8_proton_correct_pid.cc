...
void run8_proton_correct_pid( std::string list, 
  ...
  proton_1sigma.SetHarmonicArray( { 1, 2, 3, 4, 5, 6, 7, 8 } );
  proton_1sigma.SetCorrections( { CORRECTION::PLAIN } );
  ...
  proton_1sigma.AddCut( "trQ", [](double q){
    return 0.5 < q && q < 1.5;
  }, "cut on charge == 1" );
  ...
  proton_2sigma.SetHarmonicArray( { 1, 2, 3, 4, 5, 6, 7, 8 } );
  proton_2sigma.SetCorrections( { CORRECTION::PLAIN } );
  ...
  proton_2sigma.AddCut( "trQ", [](double q){
    return 0.5 < q && q < 1.5;
  }, "cut on charge == 1" );
