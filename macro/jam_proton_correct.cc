...
void jam_proton_correct(  std::string list, 
  ...
  auto simIsProton = 
  []
  (ROOT::VecOps::RVec<int> vec_pdg, ROOT::VecOps::RVec<int> vec_mother_id){
    ROOT::VecOps::RVec<int> vec_is{};
    vec_is.reserve( vec_pdg.size() );
    for( int i=0; i<vec_pdg.size(); ++i ){
      auto pdg = vec_pdg.at(i);
      auto m_id = vec_mother_id.at(i);
      if( pdg == 2212 && m_id == -1 ){
        vec_is.push_back(1);
        continue;
      }
  ...
  auto trIsParticle = 
  [](ROOT::VecOps::RVec<int> vec_sim_idx, ROOT::VecOps::RVec<int> vec_pdg ){
    ROOT::VecOps::RVec<int> vec_is( vec_sim_idx.size(), 0 );
    for( int i=0; i < vec_sim_idx.size(); ++i ){
      auto idx = vec_sim_idx.at(i);
      if( idx < 0 )
        continue;
      if( idx >= vec_pdg.size() )
        continue;
      vec_is.at(i) = vec_pdg.at(idx);
    }
    return vec_is;
  };
  auto trIsParticleTof = 
  []( ROOT::VecOps::RVec<int> vec_pdg, ROOT::VecOps::RVec<int> vec_tof400, ROOT::VecOps::RVec<int> vec_tof700 ){
    ROOT::VecOps::RVec<int> vec_is( vec_pdg.size(), 0 );
    for( int i=0; i < vec_pdg.size(); ++i ){
      auto tof400 = vec_tof400.at(i);
      auto tof700 = vec_tof700.at(i);
      auto is = vec_pdg.at(i);
      if( tof400 >= 0 || tof700 >= 0 )
        vec_is.at(i) = is;
    }
    return vec_is;
  };
  ...
  auto dd=d
          ...
          .Define( "trIsProton", trIsParticle, {"trSimIndex", "simIsProton"} )
          .Define( "trIsProtonTof", trIsParticleTof, {"trIsProton", "trTof400hit", "trTof700hit"} )
          .Define( "trProtonY", trProtonYcm, {"pz", "pq"} )
